// GPU memory pool to reduce hipMalloc overhead.
...
impl MemoryPool {
    ...
    /// Reset the pool (free all suballocations — does NOT call hipFree).  may cause error in Rocm drivers update
...
pub fn query_free_vram() -> Result<usize> {
    unsafe {
        use crate::hip_ffi::*; // check ava. VRAM
