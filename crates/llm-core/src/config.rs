...
pub struct ModelConfig {
    ...
    pub is_moe:              bool,    // true = MoE architecture
    pub num_experts:         usize,   // total experts per layer (e.g. 32 for GPT-OSS-20B)
    pub experts_per_token:   usize,   // active experts per token (top-k routing)
    pub sliding_window:      Option<usize>, // sliding window attention size
...
impl ModelConfig {
    ...
    /// GPT-OSS-20B config (OpenAI's open-weight MoE model)
    ...
    pub fn gpt_oss_20b() -> Self {
        Self {
            ...
            domain_vocab_offset: 0,
            domain_vocab_size:   0,
            hidden_dim:          2880,
            intermediate_dim:    2880,    // SwiGLU intermediate
            num_layers:          24,
            num_heads:           64,      // query heads
            num_kv_heads:        8,       // GQA key/value heads
            head_dim:            64,      // 2880 / 64 ≈ 45 (but head_dim=64 per config)
            max_seq_len:         131072,  // 128K context
            is_moe:              true,
            num_experts:         32,      // 32 experts per MoE layer
            experts_per_token:   4,       // top-4 routing
            sliding_window:      Some(128), // alternating sliding/full attention
            rms_norm_eps:        1e-5,
            use_rms_norm:        true,
            rope_theta:          150_000.0,
            rope_scaling:        Some(RopeScaling {
                factor:           32.0,
                low_freq_factor:  1.0,    // beta_slow
                high_freq_factor: 32.0,   // beta_fast
                original_max_pos: 4096,
            }),
            bos_token_id:        200000,
            eos_token_id:        200002,
            pad_token_id:        Some(199999),
            tokenizer_path:      "configs/o200k_harmony.json".into(),
            quantization: QuantizationConfig {
                method:    QuantMethod::GPTQ,  // MXFP4 in practice
                bits:      4,
                group_size: 32,
            },
            domain: DomainConfig {
                domains: vec![],  // General purpose model
                constants_db: None,
                system_prompt: GPT_OSS_SYSTEM_PROMPT.into(),
            },
            dtype: DType::BF16,
    ...
    /// PhysLLM 20B dense config (custom, NOT MoE)
    /// For training from scratch without MoE complexity
    pub fn physllm_20b() -> Self {
        Self {
            vocab_size:          36864,
            domain_vocab_offset: 32768,
            domain_vocab_size:   4096,
...
const GPT_OSS_SYSTEM_PROMPT: &str = r#"You are PhysLLM, powered by GPT-OSS-20B.
You are a specialist AI assistant with deep expertise in physics, chemistry,
astrophysics, and computational science. You reason rigorously, cite physical
constants precisely, and distinguish between established results and research frontiers."#;
